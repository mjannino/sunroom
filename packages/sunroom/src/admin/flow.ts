...
export type CallbackDecision =
  { ok: true } | { ok: false; status: 400 | 403; reason: string };
...
export function authorizeIdentity(
  ...
  if (!identity.emailVerified) {
    return {
      ok: false,
      status: 403,
      reason: "Your Google email is not verified.",
    };
  }
  if (!isEditor(config, identity.email)) {
    return {
      ok: false,
      status: 403,
      reason: "This account is not authorized to edit this site.",
    };
  }
  return { ok: true };
}
 ...
 */
export function checkOwnerToken(
