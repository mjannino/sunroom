...
describe('authorizeIdentity', () => {
  it('accepts a verified, allowlisted editor', () => {
    expect(authorizeIdentity(config, { email: 'jane@acme.com', emailVerified: true, name: 'Jane' })).toEqual({ ok: true })
  })
  it('rejects an unverified email with 403', () => {
    const d = authorizeIdentity(config, { email: 'jane@acme.com', emailVerified: false, name: 'Jane' })
    expect(d).toEqual({ ok: false, status: 403, reason: expect.stringMatching(/verified/i) })
  })
  it('rejects a non-allowlisted email with 403', () => {
    const d = authorizeIdentity(config, { email: 'mallory@evil.com', emailVerified: true, name: 'M' })
    expect(d).toEqual({ ok: false, status: 403, reason: expect.stringMatching(/authorized|allow/i) })
  })
})

describe('checkOwnerToken', () => {
  it('accepts the exact token', () => {
    expect(checkOwnerToken(config.ownerToken, 'correct-owner-token')).toBe(true)
  })
