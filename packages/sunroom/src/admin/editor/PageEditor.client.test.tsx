...
import type { ActionResult, EditorActions, SerializedRegistry } from './types.js'

const registry: SerializedRegistry = {
  hero: { label: 'Hero', fields: { heading: { type: 'text' }, body: { type: 'richText' } } },
}
const page: Page = {
  slug: 'about', title: 'About', position: 1, seo: {},
  sections: [{ id: 's1', type: 'hero', props: { heading: 'Hi', body: '<p>x</p>' } }],
}
function actionsMock(over: Partial<EditorActions> = {}): EditorActions {
  return {
    savePage: vi.fn(async (): Promise<ActionResult> => ({ ok: true, version: 'v2' })),
...
describe('PageEditor', () => {
  it('Save is disabled until an edit is made', () => {
    render(<PageEditor page={page} version="v1" registry={registry} actions={actionsMock()} />)
    expect((screen.getByRole('button', { name: /save/i }) as HTMLButtonElement).disabled).toBe(true)
  })

  it('edits a text field and saves the whole page with the base version', async () => {
    const actions = actionsMock()
    render(<PageEditor page={page} version="v1" registry={registry} actions={actions} />)
    ...
    await waitFor(() => {
      ...
      const [savedPage, baseVersion] = (actions.savePage as ReturnType<typeof vi.fn>).mock.calls[0]!
  ...
  it('shows a conflict message when save returns a conflict', async () => {
    const actions = actionsMock({ savePage: vi.fn(async (): Promise<ActionResult> => ({ ok: false, reason: 'conflict', message: 'changed elsewhere' })) })
    render(<PageEditor page={page} version="v1" registry={registry} actions={actions} />)
    fireEvent.click(screen.getByText(/Hero/))
    fireEvent.change(screen.getByLabelText('heading'), { target: { value: 'x' } })
    fireEvent.click(screen.getByRole('button', { name: /save/i }))
