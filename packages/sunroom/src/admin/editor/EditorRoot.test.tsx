...
const config = resolveConfig({
  sections: { hero: defineSection({ label: 'Hero', component: Hero, fields: { heading: f.text({ required: true }) } }) },
})
...
describe('EditorRoot dispatch', () => {
  ...
  it('renders the editor for a page slug', async () => {
    const el = await EditorRoot({ config, params: Promise.resolve({ segments: ['pages', ''] }) })
    const html = renderToStaticMarkup(el)
