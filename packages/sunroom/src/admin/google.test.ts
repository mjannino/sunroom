...
describe('buildAuthorization', () => {
  it('returns the url, state, and verifier with the right scopes', () => {
    createAuthorizationURL.mockReturnValue(new URL('https://accounts.google.com/o/oauth2/v2/auth?x=1'))
...
describe('exchangeCode', () => {
  it('normalises the id-token claims into a GoogleIdentity', async () => {
    ...
    decodeIdToken.mockReturnValue({ email: 'Jane@Acme.com', email_verified: true, name: 'Jane' })

    const identity = await exchangeCode(config, REDIRECT, 'code', 'verifier')
    expect(identity).toEqual({ email: 'Jane@Acme.com', emailVerified: true, name: 'Jane' })
  ...
  it('throws when the id token has no email', async () => {
    ...
    await expect(exchangeCode(config, REDIRECT, 'code', 'verifier')).rejects.toThrow(/email/i)
  })

  it('propagates an arctic failure', async () => {
    validateAuthorizationCode.mockRejectedValue(new Error('arctic boom'))
    await expect(exchangeCode(config, REDIRECT, 'code', 'verifier')).rejects.toThrow('arctic boom')
  })
})
