...
const UNAUTHORIZED = { ok: false, reason: 'unauthorized', message: 'You must be signed in.' } as const

function fail(e: unknown): ActionResult {
  if (e instanceof ConflictError) return { ok: false, reason: 'conflict', message: e.message }
  if (e instanceof NotFoundError) return { ok: false, reason: 'notfound', message: e.message }
  if (e instanceof ValidationError) return { ok: false, reason: 'validation', message: e.message }
  throw e
}
...
export async function createPageAction(input: { slug: string; title: string }): Promise<ActionResult> {
  ...
  if (issues.length > 0) return { ok: false, reason: 'validation', message: issues[0]!.message }

  try {
    const s = await store()
    const position = s.listPages().reduce((max, p) => Math.max(max, p.position), 0) + 1
    const page: Page = { slug: input.slug, title: input.title, position, seo: {}, sections: [] }
...
export async function deletePageAction(slug: string): Promise<ActionResult> {
  ...
  try {
    ...
    if (!existing) return { ok: false, reason: 'notfound', message: `Page "${slug}" not found.` }
...
export async function reorderPagesAction(orderedSlugs: string[]): Promise<ActionResult> {
  ...
  try {
    ...
    for (let i = 0; i < orderedSlugs.length; i++) {
      ...
      await s.savePage({ ...entry.page, position: i }, { baseVersion: entry.version, author })
      changed = true
    }
