...
import { SESSION_COOKIE, STATE_COOKIE, VERIFIER_COOKIE, verifySession } from './session.js'
...
describe('GET login', () => {
  it('redirects to the arctic authorization URL and sets txn cookies', async () => {
    const h = handlers({
      buildAuthorization: () => ({ url: 'https://accounts.google.com/auth?x=1', state: 'st', codeVerifier: 'ver' }),
    })
    const res = await h.GET(new NextRequest('https://acme.com/api/sunroom/auth/login'))
    expect(res.status).toBe(302)
    expect(res.headers.get('location')).toBe('https://accounts.google.com/auth?x=1')
...
describe('GET callback', () => {
  function callbackReq(query: string, cookies: Record<string, string>) {
    const req = new NextRequest(`https://acme.com/api/sunroom/auth/callback?${query}`)
  ...
  it('sets a session and redirects to /admin for a valid, allowlisted editor', async () => {
    const h = handlers({
      exchangeCode: async () => ({ email: 'jane@acme.com', emailVerified: true, name: 'Jane' }),
    })
    const res = await h.GET(
      callbackReq('code=abc&state=st', { [STATE_COOKIE]: 'st', [VERIFIER_COOKIE]: 'ver' }),
    )
    expect(res.status).toBe(302)
    ...
    expect(verifySession(token, config.sessionSecret)?.email).toBe('jane@acme.com')
  })

  it('rejects a state mismatch with 400 and no session', async () => {
    const h = handlers({ exchangeCode: async () => ({ email: 'jane@acme.com', emailVerified: true, name: 'Jane' }) })
    const res = await h.GET(callbackReq('code=abc&state=WRONG', { [STATE_COOKIE]: 'st', [VERIFIER_COOKIE]: 'ver' }))
  ...
  it('rejects a non-allowlisted email with 403 and no session', async () => {
    const h = handlers({ exchangeCode: async () => ({ email: 'mallory@evil.com', emailVerified: true, name: 'M' }) })
    const res = await h.GET(callbackReq('code=abc&state=st', { [STATE_COOKIE]: 'st', [VERIFIER_COOKIE]: 'ver' }))
    expect(res.status).toBe(403)
    expect(res.headers.getSetCookie().some((c) => c.startsWith(`${SESSION_COOKIE}=`))).toBe(false)
  })

  it('rejects an unverified email with 403', async () => {
    const h = handlers({ exchangeCode: async () => ({ email: 'jane@acme.com', emailVerified: false, name: 'Jane' }) })
    const res = await h.GET(callbackReq('code=abc&state=st', { [STATE_COOKIE]: 'st', [VERIFIER_COOKIE]: 'ver' }))
    expect(res.status).toBe(403)
  })
})
...
describe('POST owner', () => {
  ...
  it('mints an owner session for the correct token', async () => {
    ...
    const session = res.headers.getSetCookie().find((c) => c.startsWith(`${SESSION_COOKIE}=`))
    expect(verifySession(session!.split(';')[0]!.split('=')[1], config.sessionSecret)?.email).toBe('owner@sunroom.local')
  })
...
describe('POST logout', () => {
  it('clears the session cookie and redirects to /admin', async () => {
    const res = await handlers().POST(new NextRequest('https://acme.com/api/sunroom/auth/logout', { method: 'POST' }))
    expect(res.status).toBe(303)
    expect(res.headers.get('location')).toBe('https://acme.com/admin')
